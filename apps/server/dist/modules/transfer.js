...
let TransferService = class TransferService {
    ...
    async applyOwnership(t) {
        ...
        if (idx < 0)
            throw new common_1.BadRequestException(`谷序 #${t.seq} 不在跟排中`);
        ...
        // 检查接收方是否已有该good的跟排
        const toItems = await this.itemRepo.find({ where: { goodId: t.goodId, orderId: (0, typeorm_2.In)([order.id]) } }); // 这里查询的是同一series的所有订单
