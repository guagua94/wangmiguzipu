...
let AuctionService = class AuctionService {
    constructor(repo, bidRepo, depRepo, userRepo, balance) {
        this.repo = repo;
    ...
    /** 状态推进：到点开拍 / 到点截拍（无人出价流拍；有人出价成交）——服务端时间为准 */
    async tickStates() {
        const now = Date.now();
    ...
    async deposit(uid, auctionId, byBalance, screenshot) {
        ...
        let state = '待审核';
        if (byBalance) {
            await this.balance.debit(uid, +a.deposit, '保证金', `拍卖保证金（${a.name}）`, `A${a.id}`);
            state = '已缴';
        }
        if (!byBalance && !screenshot)
            throw new common_1.BadRequestException('请上传付款截图');
    ...
    async bid(uid, auctionId, price) {
        ...
        if (+a.buyNow > 0 && price >= +a.buyNow - 1e-9) {
            await this.repo.update(auctionId, { curPrice: (+a.buyNow).toFixed(2), bidCount: a.bidCount + 1, endTime: end, state: '待付款', winnerId: uid });
            await this.notify(uid, '拍卖中标（一口价）', `「${a.name}」你以一口价 ¥${a.buyNow} 成交！24小时内付款（保证金抵扣）`);
    ...
    async pendingDeposits() {
        ...
        const auctions = await this.repo.find({ where: { id: (0, typeorm_2.In)(auctionIds) } });
        const users = await this.userRepo.find({ where: { id: (0, typeorm_2.In)([...new Set(deps.map(d => d.userId))]) } });
        return deps.map(d => {
            const a = auctions.find(x => x.id === d.auctionId);
            const u = users.find(x => x.id === d.userId);
            return { deposit: d, auction: a, user: u };
        });
    }
    /** 审核保证金：通过（已缴），拒绝（已退） */
    ...
    async close(auctionId) {
        ...
        await this.repo.update(auctionId, { state: '待付款', winnerId: lastBid.userId });
        await this.notify(lastBid.userId, '拍卖中标', `「${a.name}」你以 ¥${lastBid.price} 中标！24小时内付款（保证金抵扣）`);
        // 其他参拍人退保证金
    ...
    async pay(uid, auctionId, screenshot, useBalanceAmount) {
        ...
        if (dep)
            await this.depRepo.update(dep.id, { state: '已抵扣' });
        const total = +a.curPrice - depAmt; // 需支付的尾款
        let useBal = Math.max(0, Math.min(useBalanceAmount || 0, total));
        let rest = total;
        if (useBal > 0) {
            await this.balance.debit(uid, useBal, '拍卖尾款', `「${a.name}」尾款（保证金抵 ¥${depAmt.toFixed(2)}，余额抵 ¥${useBal.toFixed(2)}）`, `A${a.id}`);
            rest = total - useBal;
        }
        if (rest > 0) {
            if (!screenshot)
        ...
        await this.repo.update(auctionId, { state: '已付款' });
        ...
        await this.notify(uid, '拍卖付款完成', `「${a.name}」已付款（保证金抵 ¥${depAmt.toFixed(2)}${useBal > 0 ? `，余额抵 ¥${useBal.toFixed(2)}` : ''}）`);
...
let AuctionController = class AuctionController {
    ...
    async deposit(req, b) {
        ...
        return this.svc.deposit(req.user.id, b.auctionId, b.byBalance, b.screenshot || '');
    }
    async bid(req, b) {
        (0, common_2.checkRole)(req.user, []);
...
exports.AuctionModuleRef = typeorm_1.TypeOrmModule.forFeature([entities_1.Auction, entities_1.AuctionBid, entities_1.AuctionDeposit, entities_1.User]);
