...
let SaleService = class SaleService {
    ...
    async list(q) {
        ...
        return r.filter(g => g.stock > 0 || true).sort((a, b) => b.id - a.id);
    }
    async save(b) {
        if (b.id) {
    ...
    async buy(uid, goodId, qty) {
        return this.batchBuy(uid, [{ goodId, qty }]);
    }
    /** 批量购买：多个直售谷子合为单笔订单，一起结算 */
    async batchBuy(uid, items) {
        ...
        for (const it of items) {
            ...
            const g = await this.repo.findOneByOrFail({ id: it.goodId });
            if (g.stock < it.qty)
        ...
        for (const oi of orderItems) {
            await this.itemRepo.save(this.itemRepo.create({ orderId: order.id, ...oi }));
        }
        const nRepo = this.repo.manager.getRepository(entities_1.Notification);
    ...
    async audit(orderId, pass) {
        ...
        if (o.status !== '已提交截图')
            throw new common_1.BadRequestException('状态错误');
        await this.orderRepo.update(orderId, { status: pass ? '囤货中' : '待付款' });
        return { ok: true };
    }
    /** 团员申请取消直售订单 */
...
let SaleController = class SaleController {
    ...
    async batchBuy(req, b) {
        (0, common_2.checkRole)(req.user, []);
        return this.svc.batchBuy(req.user.id, b.items);
    }
    async pay(req, b) {
        (0, common_2.checkRole)(req.user, []);
