...
let ClearingService = class ClearingService {
    constructor(repo, userRepo, orderRepo, itemRepo, goodRepo, saleRepo, addrRepo, shop, balance) {
        this.repo = repo;
    ...
    async create(uid, b) {
        ...
        const items = await this.itemRepo.find({ where: { orderId: (0, typeorm_2.In)(b.orderIds) } });
        // 获取收货地址快照
        let addressSnapshot = '';
        if (b.addressId) {
        ...
        for (const o of orders) {
            ...
            if (!feeOn)
                continue;
            const daysPassed = Math.floor((now - created) / (24 * 3600 * 1000));
        ...
        const c = await this.repo.save(this.repo.create({
            userId: uid,
            items: JSON.stringify(items.map(i => ({ name: i.name, qty: i.qty }))),
    ...
    async ship(id, trackingNo, packImg) {
        await this.repo.update(id, { state: '已发货', trackingNo, packImg });
        const c = await this.repo.findOneByOrFail({ id });
...
exports.ClearingModuleRef = typeorm_1.TypeOrmModule.forFeature([entities_1.Clearing, entities_1.User, entities_1.Order, entities_1.OrderItem, entities_1.Good, entities_1.SaleGood, entities_1.Address]);
